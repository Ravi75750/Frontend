    ...
    const checkPayment = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL || "https://backend-d10xvopad-ravi-sahanis-projects.vercel.app/api"}/payments/history/${uid}`
        );
  ...
  const joinContestNow = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL || "https://backend-d10xvopad-ravi-sahanis-projects.vercel.app/api"}/contests/${contestId}/join`,
        { userId: uid }
      );
