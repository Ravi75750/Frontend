...
export default function VerifyPayments() {
  ...
  const loadPayments = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://backend-d10xvopad-ravi-sahanis-projects.vercel.app/api"}/payments/pending`, {
  ...
  const updateStatus = async (id, status) => {
    try {
      setUpdatingId(id);
      await axios.put(
        `${import.meta.env.VITE_API_URL || "https://backend-d10xvopad-ravi-sahanis-projects.vercel.app/api"}/payments/update/${id}`,
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      );
